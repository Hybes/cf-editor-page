...
export default defineEventHandler(async (event) => {
  try {
    ...
    const probe = async (path, featureName) => {
      ...
      return {
        available: false,
        reason: (data && data.errors && data.errors[0] && data.errors[0].message) || `${featureName} unavailable`,
      };
    };
    ...
    const probeRulesets = async (zoneId) => {
      const list = await cfFetch({ apiKey: body.apiKey, method: 'GET', path: `/zones/${zoneId}/rulesets?per_page=1` });
      ...
      const detail = await cfFetch({ apiKey: body.apiKey, method: 'GET', path: `/zones/${zoneId}/rulesets/${first.id}` });
      if (detail && detail.success) return { available: true, reason: '' };

      return {
        available: false,
        reason: (detail && detail.errors && detail.errors[0] && detail.errors[0].message) || 'Rulesets unavailable',
      };
    };
    ...
    if (body.currZone) {
      ...
      result.zone = zoneData && zoneData.success ? { available: true, reason: '' } : {
        available: false,
        reason: (zoneData && zoneData.errors && zoneData.errors[0] && zoneData.errors[0].message) || 'Zone unavailable',
      };
      ...
      const accountId = zoneData && zoneData.success && zoneData.result && zoneData.result.account && zoneData.result.account.id
