<template>
	<div>
		...
		<div class="w-full">
			<div class="flex flex-row flex-wrap items-center justify-center gap-2 px-4 py-4 md:justify-between">
				<div class="flex flex-row flex-wrap justify-center gap-2">
					<UButton @click="clearZone()" variant="outline" icon="i-clarity-undo-line" to="/"> Back </UButton>
					<UButton to="/create" variant="outline" color="green" icon="i-clarity-plus-circle-solid">
						Create
					</UButton>
				</div>
				<div class="flex flex-row flex-wrap justify-center gap-2">
					<UButton @click="sort('type')" variant="outline" color="white" icon="i-clarity-cloud-network-line"
						>Sort Type</UButton
					>
					<UButton @click="sort('name')" variant="outline" color="white" icon="i-clarity-sort-by-line"
						>Sort Name</UButton
					>
			...
			<div v-else>
				...
				<div v-if="columns === false">
					<div
						:class="{
							'sm:grid-cols-1': dnsRecords.length < 2,
							'sm:grid-cols-2': dnsRecords.length > 1,
						}"
						class="grid grid-cols-1 gap-4 p-4"
					>
						<div
							@click="setDns(record)"
							class="cursor-pointer overflow-hidden rounded bg-stone-200 px-4 py-2 hover:bg-stone-300 dark:bg-stone-800 dark:hover:bg-stone-900"
				...
				<div v-if="columns === true || columns === null">
					<div class="flex w-full flex-col flex-wrap justify-center gap-4 overflow-clip p-4">
						<div
						...
						>
							<div class="flex w-[calc(100%-3.8rem)] gap-6" @click="setDns(record)">
								<div class="min-w-[6rem] px-2 py-1 text-center">
									<p class="font-bold">{{ record.type }}</p>
								</div>
								<div
									class="border-l-2 border-stone-600 px-2 py-1 pl-8 text-center dark:border-stone-400"
								>
									<p>{{ record.name }}</p>
								</div>
								<div
									class="max-w-[600px] overflow-hidden border-l-2 border-stone-600 px-2 py-1 pl-8 text-center dark:border-stone-400"
								>
									<p class="truncate">{{ record.content }}</p>
								</div>
							</div>
